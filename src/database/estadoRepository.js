...
class EstadoRepository {
    ...
    async listarEstados() {
        try {
            const res = await pool.query(`SELECT estado FROM bot_estado_config WHERE ativo = TRUE`);
    ...
    async registrarTransicao(chatId, estadoAnterior, estadoNovo, mensagemGatilho = null) {
        ...
        `;
        try {
            await pool.query(query, [chatId, estadoAnterior, estadoNovo, mensagemGatilho]);
        } catch (err) {
