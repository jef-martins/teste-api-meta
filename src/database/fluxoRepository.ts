...
class FluxoRepository {
  ...
  async obterFluxo(id) {
    const res = await pool.query(`SELECT * FROM bot_fluxo WHERE id = $1`, [id]);
  ...
  async ativarFluxo(id) {
    ...
    try {
      ...
      await client.query(
        `UPDATE bot_estado_config SET ativo = true WHERE flow_id = $1`,
        [id],
      );
      ...
      await client.query(
        `UPDATE bot_estado_transicao SET ativo = true
                 WHERE estado_origem IN (SELECT estado FROM bot_estado_config WHERE flow_id = $1)`,
        [id],
      );
      ...
      const { rows: startRows } = await client.query(
        `SELECT estado FROM bot_estado_config WHERE flow_id = $1 AND node_type = 'start' LIMIT 1`,
        [id],
      );
      ...
      if (estadoInicial) {
        await client.query(
          `UPDATE bot_estado_usuario SET estado_atual = $1, atualizado_em = NOW()`,
          [estadoInicial],
        );
      } else {
  ...
  async limparEstadosDoFluxo(flowId) {
    ...
    await pool.query(`DELETE FROM bot_estado_config WHERE flow_id = $1`, [
      flowId,
    ]);
  }
  ...
  async salvarVariaveisDoFluxo(flowId, variaveis) {
    // Limpa existentes
    await pool.query(`DELETE FROM bot_fluxo_variaveis WHERE flow_id = $1`, [
      flowId,
    ]);
