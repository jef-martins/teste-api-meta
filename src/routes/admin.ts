...
router.delete('/estados/:estado', async (req, res) => {
  try {
    await pool.query(`DELETE FROM bot_estado_config WHERE estado=$1`, [
      req.params.estado,
    ]);
    res.json({ ok: true });
  } catch (err) {
...
router.post('/testar-req', async (req, res) => {
  ...
  const interpolar = (texto, varData) =>
    typeof texto === 'string'
      ? texto.replace(/\{(\w+)\}/g, (_, k) => varData[k] ?? `{${k}}`)
      : texto;
  ...
  try {
    ...
    if (usandoBodyFixo) {
      ...
      bodyObj = {
        [config.campoEnviar]: valor || (variaveis && variaveis.valor) || '',
      };
    } else {
      bodyObj = { ...tudo };
    }
    ...
    if (metodo === 'GET') {
      ...
      if (metodo === 'GET') {
        const searchParamsBody = Object.fromEntries(
          Object.entries(bodyObj).filter(
            ([_, v]) => v !== undefined && v !== '',
          ),
...
router.delete('/transicoes/:id', async (req, res) => {
  try {
    await pool.query(`DELETE FROM bot_estado_transicao WHERE id=$1`, [
      req.params.id,
    ]);
    res.json({ ok: true });
  } catch (err) {
