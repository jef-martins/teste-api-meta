...
router.post('/fluxos', async (req, res) => {
    try {
        ...
        const { estadosPrefixados, transicoesAtualizadas } = aplicarPrefixo(fluxo.id, estados, transicoes);
...
router.put('/fluxos/:id', async (req, res) => {
    try {
        const { name, description, nodes, connections, variables, version } = req.body;
        const flowId = req.params.id;

        const fluxoExistente = await fluxoRepo.obterFluxo(flowId);
        if (!fluxoExistente) return res.status(404).json({ erro: 'Fluxo não encontrado' });
        ...
        const { estados, transicoes, variaveis } = flowToStateMachine(flowJson);
        const { estadosPrefixados, transicoesAtualizadas } = aplicarPrefixo(flowId, estados, transicoes);
