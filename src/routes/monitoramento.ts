...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const {
      rows: [{ count: totalSessoes }],
    } = await pool.query(`SELECT COUNT(*) FROM bot_estado_usuario`);

    // Sessões ativas hoje
    const {
      rows: [{ count: sessoesHoje }],
    } = await pool.query(
    ...
    // Total de mensagens
    const {
      rows: [{ count: totalMensagens }],
    } = await pool.query(`SELECT COUNT(*) FROM conversa`);

    // Mensagens hoje
    const {
      rows: [{ count: mensagensHoje }],
    } = await pool.query(
    ...
    // Fluxos salvos
    const {
      rows: [{ count: totalFluxos }],
    } = await pool.query(`SELECT COUNT(*) FROM bot_fluxo`);
