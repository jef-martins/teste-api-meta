...
function convertVariablesDeep(obj, converter) {
    ...
    if (Array.isArray(obj)) return obj.map(item => convertVariablesDeep(item, converter));
    if (typeof obj === 'object' && obj !== null) {
        return Object.fromEntries(
            Object.entries(obj).map(([k, v]) => [k, convertVariablesDeep(v, converter)])
        );
    }
    return obj;
}
...
function flowToStateMachine(flowJson) {
    ...
    const transicoes = connections.map(conn => {
...
function messageNodeToHandler(props, subs) {
    ...
    if (waitForResp) {
        ...
        const mensagens = sendMessages.map(s => s.properties?.content).filter(Boolean);
...
function stateMachineToFlow(estados, transicoes, variaveis = []) {
    ...
    let connIdx = 0;
    const connections = transicoes.map(t => {
...
function handlerToNodeType(handler, config) {
    switch (handler) {
        case '_handlerMensagem':
            if (config?.transicaoAutomatica && (!config.mensagens || config.mensagens.length === 0)) {
                return 'start';
            }
