...
class Handler extends Bot {
  ...
  async _handlerMensagem(message, chatId, corpo, engine) {
    ...
    const config =
      (await estadoRepository.obterConfigEstado(estadoAtual))?.config ?? {};
  ...
  async _handlerCapturar(message, chatId, corpo, engine) {
    ...
    if (Array.isArray(config.campos) && config.campos.length > 0) {
      return await this._handlerCapturarMulti(
        message,
        chatId,
        corpo,
        estadoAtual,
    ...
    let proximo = await estadoRepository.buscarProximoEstado(
      estadoAtual,
      corpo,
    );
    if (
  ...
  async _handlerCapturarMulti(
    ...
    if (!proximoCampo) {
      ...
      console.warn(
        `[Bot] [${chatId}] _handlerCapturarMulti: todos os campos já preenchidos.`,
      );
      return;
    }
    ...
    const proximo = await estadoRepository.buscarProximoEstado(
      estadoAtual,
      '*',
    );
    if (!proximo) {
      console.warn(
        `[Bot] [${chatId}] _handlerCapturarMulti: nenhuma transição '*' configurada para ${estadoAtual}.`,
      );
      return;
    }
  ...
  async _handlerLista(message, chatId, corpo, engine) {
    ...
    console.log(
      `[Bot] [${chatId}] sendListMessage → destino=${destino} | opções=${opcoes.length}`,
    );
  ...
  async _handlerBotoes(message, chatId, corpo, engine) {
    ...
    try {
      await (this as any).client.sendText(
        message.from,
