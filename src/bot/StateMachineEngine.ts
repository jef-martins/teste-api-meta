...
class StateMachineEngine {
  ...
  async process(
    ...
    if (estadoSalvo && !this._estadosAvisados?.has(chatId)) {
      console.log(
        `[Engine] [${chatId}] estado restaurado do banco: ${estadoSalvo}`,
      );
    ...
    if (!config) {
      console.warn(
        `[Engine] Estado "${estadoAtual}" não encontrado/ativo no banco. Reiniciando para ${estadoPadrao}.`,
      );
    ...
    console.log(
      `[Engine] [${chatId}] estado=${estadoAtual} → handler=${config.handler}`,
    );
    ...
    if (config.config?.aguardarEntrada && entrada) {
      console.log(
        `[Engine] [${chatId}] estado aguarda entrada → buscando transição para "${entrada}"`,
      );
    ...
    if (typeof this.actionDelegate[config.handler] === 'function') {
      ...
      console.error(
        `[Engine] Handler "${config.handler}" não existe no Delegate (Bot)!`,
      );
    }
  }
  ...
  async transitarPorEntrada(
    ...
    if (executarHandler) {
      ...
      if (
        configProximo &&
        typeof this.actionDelegate[configProximo.handler] === 'function'
      ) {
        await this.actionDelegate[configProximo.handler](
          message,
          chatId,
          '',
          this,
