...
class BotWhatsApp {
  ...
  async _enviarMensagemDeInicializacao() {
    ...
    try {
      ...
      console.error(
        '[Bot] Erro ao enviar mensagem de inicialização:',
        err.message,
      );
    }
  }
  ...
  _ouvirMensagens() {
    this.client.onMessage(async (message) => {
      ...
      console.log(
        `[Bot] Mensagem recebida de ${message.from} [${message.type}]: ${message.body ?? ''}`,
      );
      ...
      try {
        ...
        console.error(
          `[Bot] ❌ Erro ao processar mensagem de ${message.from}:`,
          err.message,
        );
  ...
  async _processarMensagem(message) {
    throw new Error(
      `[BotWhatsApp] _processarMensagem() não implementado na subclasse.`,
    );
  }
  ...
  async enviarMensagem(numero, texto) {
    ...
    try {
      ...
      console.error(
        `[Bot] Falha no sendText para ${numero}:`,
        err.message || err,
      );
      throw err;
    }
  ...
  async enviarResposta(mensagemOriginal, textoResposta) {
    try {
      ...
      await conversaRepository.salvarMensagem(
        'WPPConnect Bot',
        resultado,
        quemEnviou,
        paraQuem,
