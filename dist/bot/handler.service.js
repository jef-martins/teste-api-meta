...
let HandlerService = HandlerService_1 = class HandlerService {
    ...
    async _handlerBotoes(message, chatId, corpo, engine) {
        ...
        try {
            await this.client.sendText(message.from, config.titulo ?? 'Escolha uma opção:', {
                ...
                buttons: (config.botoes ?? []).map((b) => ({ id: b.entrada, text: b.label })),
            });
        }
        catch (err) {
    ...
    async _handlerRequisicao(message, chatId, corpo, engine) {
        ...
        try {
            ...
            const tudo = { id: crypto.randomUUID(), valor: corpo, chatId, from, numero, ...dadosMemoria };
            ...
            if (usandoBodyFixo) {
                ...
                bodyObj = Object.fromEntries(config.camposEnviar.map((chave) => [chave, dadosMemoria[chave] ?? '']));
            }
                ...
                if (valorExtraido === '' || valorExtraido === null || valorExtraido === undefined) {
                    ...
                    if (valorExtraido.length === 0) {
                        ...
                        const partes = valorExtraido.map((item) => {
                            const objLimpo = Object.fromEntries(Object.entries(item).map(([k, v]) => [k, typeof v === 'string' ? v.replace(/<[^>]+>/g, ' ').trim() : v]));
                    ...
                    if (typeof valorExtraido === 'object' && valorExtraido !== null) {
                        const objLimpo = Object.fromEntries(Object.entries(valorExtraido).map(([k, v]) => [k, typeof v === 'string' ? v.replace(/<[^>]+>/g, ' ').trim() : v]));
